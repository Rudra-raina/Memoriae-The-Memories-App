...
import com.example.memoriae.databinding.SearchRvItemBinding
...
class SearchMemoryAdapter(private val context : Context, private var list : ArrayList<Memory>) :
    RecyclerView.Adapter<SearchMemoryAdapter.MyViewHolder>(){

    class MyViewHolder ( val binding : SearchRvItemBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(SearchRvItemBinding.inflate(LayoutInflater.from(context), parent, false))
    }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.binding.cityName.text =model.cityName.toUpperCase()
        holder.binding.tvDescription.text = model.description
        holder.itemView.tvDate.text = model.date
        holder.itemView.tvName.text="Review by ${model.userName}"
